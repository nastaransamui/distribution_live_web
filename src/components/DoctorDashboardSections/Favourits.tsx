...
const Favourits: FC = (() => {
  ...
  useEffect(() => {
    ...
    if (isActive && homeSocket.current !== undefined && userProfile !== null) {
      ...
      homeSocket.current.emit('getFavPatientsForDoctorProfile', { userId, favIdArray, paginationModel, sortModel, mongoFilterModel, })
      homeSocket.current.once('getFavPatientsForDoctorProfileReturn', (msg: {
        status: number,
        userFavProfile: FavPatientProfile[],
        message?: string;
      }) => {
        ...
        if (status !== 200) {
          ...
          if (userFavProfile.length !== 0) {
            const { patients, totalCount } = userFavProfile[0]
