...
import { formatNumberWithCommas, StyledBadge, TimeType } from '@/components/DoctorDashboardSections/ScheduleTiming';
...
interface CheckoutComponentProps {
  checkoutDataServer: CheckoutDataType;
}
const Checkout: FC<CheckoutComponentProps> = (({ checkoutDataServer }) => {

  const router = useRouter()
  const theme = useTheme();
  const [termsDialog, setTermsDialog] = useState<boolean>(false)
  const [checkoutData, setCheckoutData] = useState<CheckoutDataType>(checkoutDataServer);
  ...
  const [paymentInfo, setPaymentInfo] = useState<any>({
    ...
    totalPrice: checkoutDataServer?.timeSlot.total.toFixed(2),
    currencyCode: checkoutDataServer?.timeSlot?.currencySymbol || 'THB',
    countryCode: checkoutDataServer?.doctorProfile?.currency[0]?.iso2 || 'TH',
  })
    ...
    <Fragment>
      {
        isLoading ?
          ...
          <div className={`col-md-6 col-lg-7   ${isClient ? 'animate__animated animate__backInLeft' : 'pre-anim-hidden'}`} style={{ ...muiVar, position: 'relative' }}>
