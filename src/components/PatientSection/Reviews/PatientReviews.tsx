...
import React, { FC, useState, useEffect, Fragment, MouseEvent, ReactNode, useRef } from 'react';
...
const PatientReviews: FC<PatientReviewsType> = (({ profile }) => {
  ...
  useEffect(() => {
    let isMounted = true;
    if (!profile || !homeSocket?.current || !isMounted) return;
    setIsLoading(true)

    const socket = homeSocket.current;
    const handleReviews = (msg: { status: number, authorReviews: ReviewTypes[], totalReviews: number, message?: string }) => {
      if (msg?.status !== 200) {
        ...
        socket.on(`updateGetAuthorReviews`, () => {
          setReload(!reload)
        })
        setIsLoading(false)
      }
    ...
    if (homeSocket?.current) {
      socket.emit('getAuthorReviews', { patientId: profile?._id, paginationModel, sortModel, mongoFilterModel })
