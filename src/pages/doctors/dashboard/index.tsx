...
import DashboardMain from '@/components/DoctorDashboardSections/DashboardMain';
  ...
  (store) => async (ctx) => {
    try {
      ...
      if (hasCookie('homeAccessToken', ctx)) {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_adminUrl}/api/singleDoctor?_id=${user_id}`, {
          method: "GET",
          headers: {
            'Accept': 'application/json',
...
export default connect((state: AppState) => state)(DashboardPage);
